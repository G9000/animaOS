// apps/animus/src/tools/process_manager.ts
...
//
// Unlike letta's TaskOutput/TaskStop which spawns new letta instances,
  ...
  if (proc.stdout) {
    (async () => {
      ...
      while (true) {
        ...
        if (done) break;
        pushLines(decoder.decode(value, { stream: true }));
      }
    })().catch(() => {});
  }
  if (proc.stderr) {
    (async () => {
      ...
      while (true) {
        const { done, value } = await reader.read();
        if (done) break;
        pushLines("[stderr] " + decoder.decode(value, { stream: true }));
      }
    })().catch(() => {});
  }

  proc.exited.then((code) => { managed.exitCode = code; }).catch(() => {});
