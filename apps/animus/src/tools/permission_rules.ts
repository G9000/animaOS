// apps/animus/src/tools/permission_rules.ts
// Load user-defined permission rules from project-local or global config.
//
// File locations (checked in order, merged):
//   .animus/permissions.json   (project-local)
//   ~/.animus/permissions.json (global)
//
// Format:
// {
//   "allow": [
...
export function loadPermissionRules(cwd: string = process.cwd()): PermissionRules {
  const local = loadFile(join(cwd, ".animus", "permissions.json"));
  const global = loadFile(join(homedir(), ".animus", "permissions.json"));
