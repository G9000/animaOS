// apps/animus/src/tools/secrets.ts
// Secret substitution + redaction for tool arguments and output.
//
// Improvements over letta's version:
//  - Falls back to process.env when .animus/secrets.json doesn't exist
...
function secretsFilePath(): string {
  return join(homedir(), ".animus", "secrets.json");
}

/**
 * Load secrets from ~/.animus/secrets.json, falling back to process.env
...
export function redactSecrets(input: string): string {
  const secrets = loadSecrets();
  // Also include any env vars that were actually used (intersection of env + pattern matches)
