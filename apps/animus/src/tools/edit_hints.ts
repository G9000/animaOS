...
function hasSmartQuoteMismatch(search: string, content: string): boolean {
  ...
  // Normalize the search string by trying all curly-quote combinations.
  // We use a token-based approach: replace each " with \u201C or \u201D
  // and each ' with \u2018 or \u2019, then check if any variant appears in content.
