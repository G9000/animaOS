// apps/animus/src/tools/truncation.ts
...
const OVERFLOW_MAX_AGE_MS = 4 * 60 * 60 * 1000; // 4 hours — tighter than letta's 24h

function overflowDir(): string {
  return join(homedir(), ".animus", "overflow");
}
...
export function writeOverflow(content: string, toolName: string): string {
  ...
  // Auto-expire old files opportunistically (cheap scan, no separate cron)
