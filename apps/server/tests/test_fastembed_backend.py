...
def test_successful_load_clears_its_own_latch(monkeypatch) -> None:
    ...
    assert second == [[1.0] * 4]
    assert fastembed_backend._failed == {}
...
# ── Per-model failure cooldowns — a second model's failure must not evict ──
#   the first's. The failure latch is a dict keyed by model name (copy-on-
#   write for lock-free reads), so two models that fail within the TTL window
#   each keep their own cooldown; switching back to an earlier bad name still
#   respects its remaining cooldown instead of re-attempting the load.


def test_failed_latch_preserves_each_models_cooldown(monkeypatch) -> None:
    clock = {"now": 1000.0}
    monkeypatch.setattr(fastembed_backend.time, "monotonic", lambda: clock["now"])
    ...
    assert fastembed_backend.embed_texts(["x"], model_name="model-a") == [None]
    assert fastembed_backend._cooldown_active("model-a") is True

    # model-b fails while model-a is STILL within its 300s window.
    clock["now"] = 1100.0
    assert fastembed_backend.embed_texts(["y"], model_name="model-b") == [None]

    # Both cooldowns coexist — model-b's failure did not overwrite model-a's.
    assert fastembed_backend._failed == {"model-a": 1000.0, "model-b": 1100.0}
    assert fastembed_backend._cooldown_active("model-a") is True
    assert fastembed_backend._cooldown_active("model-b") is True

    # Retrying model-a while it's still cooling down makes NO load attempt.
    calls: list[str] = []
    ...
    monkeypatch.setattr(fastembed_backend, "_create_model", counting_factory)
    assert fastembed_backend._load_model("model-a") is None
    assert calls == []

    # Once model-a's OWN cooldown lapses it retries again (independently of
    # model-b, whose window is unrelated).
