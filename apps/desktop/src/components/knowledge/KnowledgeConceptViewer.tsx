...
export default function KnowledgeConceptViewer({
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center font-mono text-caption uppercase tracking-caps-3 text-muted-foreground/40">
        Loading concept
      </div>
    );
  }
  ...
  if (!concept) {
    return (
      <div className="flex h-full items-center justify-center font-mono text-caption uppercase tracking-caps-3 text-muted-foreground/35">
        Select a concept
      </div>
    );
  }
    ...
    <article className="flex h-full min-h-0 flex-col">
      <header className="shrink-0 border-b border-border px-6 py-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className="font-mono text-label text-muted-foreground/35">
            {concept.slug}
          </span>
        </div>
      </header>
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-border bg-card/30">
          ...
          <div className="divide-y divide-border/70">
            {concept.citations.length === 0 ? (
              <p className="px-4 py-5 font-mono text-caption uppercase tracking-caps-2 text-muted-foreground/35">
                No citations
              </p>
            ) : (
              concept.citations.map((citation) => (
                <div key={citation.id} className="px-4 py-3">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-label uppercase tracking-caps-2 text-primary/80">
                  ...
                  <p className="mt-2 truncate font-mono text-label text-muted-foreground/35">
