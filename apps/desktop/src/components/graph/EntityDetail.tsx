                ...
                {entity.outgoingRelations.map((rel) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className={`font-mono text-label tracking-wider ${getRelationColor(rel.type)}`}>
                        {rel.type}
                      </span>
                      <span className="font-mono text-caption text-foreground">→ {rel.target?.name}</span>
                      <span className="font-mono text-micro text-muted-foreground/40">
                        ({rel.target?.type})
                      </span>
                    </div>
                    <span className="font-mono text-label text-muted-foreground/30">{rel.mentions}M</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                {entity.incomingRelations.map((rel) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-caption text-foreground">{rel.source?.name}</span>
                      <span className="font-mono text-micro text-muted-foreground/40">
                        ({rel.source?.type})
                      </span>
                      <span className={`font-mono text-label tracking-wider ${getRelationColor(rel.type)}`}>
