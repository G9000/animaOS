...
import { cn, Button, InfoIcon, glass } from "@anima/standard-templates";
    ...
    <div className="flex flex-col items-center gap-3" ref={bottomRef}>
      ...
      <div className={cn(glass, "animate-fade-in")}>
        ...
        <div className="bg-accent px-3 py-1">
          <p className="font-mono text-ui font-semibold tracking-caps-4 text-foreground uppercase">
            {COPY.recoveryLabel}
          </p>
        </div>

        {/* Hint row */}
        <div className="flex items-start gap-2 px-3 py-2 border-b border-hairline">
        ...
        <div className="grid grid-cols-3 font-mono">
          {words.map((word, i) => (
            <div
              key={i}
              className="flex items-baseline gap-2 px-3 py-2 animate-fade-in border-hairline [&:not(:nth-child(3n))]:border-r [&:not(:nth-last-child(-n+3))]:border-b"
        ...
        <button
          ...
          className="w-full px-3 py-2 font-mono text-detail text-muted-foreground/60 uppercase tracking-widest border-t border-hairline hover:text-accent hover:bg-accent/10 transition-colors cursor-pointer text-center"
        >
