  ...
  return (
    <div className={`${glass} p-6 space-y-5`}>
      ...
      <div className="space-y-1.5">
        <h3 className="font-mono text-label tracking-caps-3 uppercase text-foreground/30">Vault Passphrase</h3>
        <input
          type="password"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <label className="font-mono text-label tracking-caps-3 uppercase px-4 py-2.5 border border-hairline text-foreground/40 hover:border-hairline-strong hover:text-foreground/70 transition-all cursor-pointer">
          Load File
      ...
      <div className="space-y-1.5">
        <h3 className="font-mono text-label tracking-caps-3 uppercase text-foreground/30">Vault Payload</h3>
        <textarea
          value={vaultPayload}
      ...
      {vaultStatus && (
        <p className="font-mono text-caption text-foreground/40 tracking-wide">{vaultStatus}</p>
      )}
    </div>
  );
}
...
function ActionButton({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="font-mono text-label tracking-caps-3 uppercase px-4 py-2.5 border border-hairline text-foreground/40 hover:border-hairline-strong hover:text-foreground/70 disabled:opacity-30 transition-all"
    >
      {children}
    </button>
  );
