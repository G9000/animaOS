...
**Acceptance**: a simulated 3-week gap produces the same closed-form state (affect, pressures) as 30,240 individual ticks within float tolerance (threshold-indicator state — the allostatic accumulator — matches within one tick quantum, as it is quantization-dependent by construction) — dream effects excluded from the equivalence, since catch-up intentionally defers to a single dream; startup catch-up completes < 50 ms with zero LLM calls; no behavioral output is generated during catch-up itself (deferred to normal gating).
