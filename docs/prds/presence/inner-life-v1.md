...
**Acceptance**: a simulated 3-week gap produces the same closed-form state (affect, pressures) as 30,240 individual ticks within float tolerance (threshold-indicator state — the allostatic accumulator — matches within one tick quantum, and the baseline-shift feed-through into arousal within the tick quantization of shift updates, as both are quantization-dependent by construction) — dream effects excluded from the equivalence, since catch-up intentionally defers to a single dream; startup catch-up completes < 50 ms with zero LLM calls; no behavioral output is generated during catch-up itself (deferred to normal gating).
...
- **New candidate scoring/rejection flow** (does not exist today — `plan_candidate_promotion()` currently promotes, reinforces, supersedes, or evolves every candidate; nothing is rejected by score). IL4 adds it: each `MemoryCandidate` gets a normalized score `s = clamp01(0.6 · importance/5 + 0.3 · emotional_salience + 0.1 · evidence_strength)` and a promotion threshold `θ_p` (config; default calibrated so importance ≥ 2 promotes exactly as today — the flow must be behavior-preserving for current candidates).
- The extraction prompt additionally emits `minor_observation` candidates it is currently instructed to omit — this is where sub-threshold volume actually comes from, since today weak signals die at extraction time, not at promotion time.
- Candidates with `s ∈ [0.25 · θ_p, θ_p)` are folded into a latent trace additively: `weight ← min(1.0, weight + s · 0.5)` per matching topic, evidence ref appended. The update is a leaky integrator — additive growth so repeated weak signals genuinely accumulate (an EMA would converge to the candidate score and never crystallize), with the weekly decay below as the leak.
