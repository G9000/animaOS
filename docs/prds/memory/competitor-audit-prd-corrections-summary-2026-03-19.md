...
- `F4`: Added an explicit graph lifecycle, clarified that `graph_ingestion` is a separate structured background task rather than a nested consolidation step, kept extraction and stale-relation pruning bounded to current-turn entities and explicit relation deltas, required lightweight local BM25 reranking for graph triples, and made the SQLite/Core storage constraint explicit. This addresses the audit finding that competitor graph patterns had been imported too loosely and needed tighter lifecycle and architecture boundaries.
- `F5`: Kept the useful Letta-inspired orchestration mechanics while explicitly rejecting open-ended background agents, defined the completed-run restart cursor contract in `BackgroundTaskRun.result_json`, aligned `graph_ingestion` with F4 as its own parallel structured task with current-turn / explicit-delta scope instead of transcript-wide graph scans, and updated the metadata wording to reflect both turn-frequency and inactivity-triggered runs. This addresses the audit finding that the PRD needed tighter orchestration boundaries and concrete restart/audit semantics.
