...
- Completed: 2026-07-15 21:58 MYT
...
- 2026-07-15 21:56 MYT - Reopened `RWF-004` after quality review found that `AGENTS.md` still described root development as Nx-only and retained the removed legacy API in its language baseline; preserved prior completion `2026-07-15 21:45 MYT`, cleared the current completion, and synchronized the parent row/history for repair.
- 2026-07-15 21:58 MYT - Re-completed `RWF-004` after grounding contributor development, build, and language guidance in root `package.json`, `dev-root` launcher specs, and every current language-family manifest; revalidated 44 focused tests, the live organization check, diff, and source exclusion, then synchronized the parent row and one current completed-history entry.
...
- Commands:
  ...
  - PowerShell JSON checks for root `scripts.dev`/`scripts.build`, read-only Bun `buildTargetSpec` assertion for `server`/`desktop`/`anima-mod`, and manifest-existence checks for current Python, TypeScript, and Rust projects
...
- Notes:
  ...
  - quality follow-up passed 44 tests with 93 assertions and 0 failures; live `check:repo` passed
  - root `dev` is exactly `node ./scripts/dev-root.mjs`; launcher specs resolve the server to `uv` and desktop/anima-mod to `bun`, matching the corrected contributor command map
  - language guidance is grounded in 14 current app/package manifests and contains no legacy `apps/api` baseline
