...
- Simulated 3-week gap equals 30,240 individual ticks within float tolerance for closed-form state (threshold-indicator state — the allostatic accumulator — matches within one tick quantum, and the baseline-shift feed-through into arousal within the tick quantization of shift updates, as both are quantization-dependent by construction; dream effects excluded by design) (tests — `test_catchup_equivalence_over_three_weeks` from arousal 0.9 exercising the threshold crossing, `test_catchup_equivalence_below_threshold_regime` at 1e-6 for the pure-exponential regime, `test_catchup_decays_baseline_shift_over_gap` for the nonzero-shift regime at 1e-4).
...
- 2026-07-17 - Final review round: catch-up no longer freezes `arousal_baseline_shift` across the gap (pre-fix arousal error: the full 0.05 max shift on a 21-day gap from shift 0.05/load 60 h). `affect.relax_with_shift_dynamics` absorbs the segment's shift law into the arousal solution as a closed-form particular term (exponential decay: F·e^(−t/τs)/(1−τa/τs); linear f(load): α+β(t−τa)); `apply_idle_gap` segments the gap at t* (bisection) and at algebraic load crossings of the sustained threshold/cap — ≤ a handful of segments, all O(1). `relax` docstring softened: semigroup exactness holds per fixed-shift segment; `apply_idle_gap` owns segmenting. Also: `system_zoneinfo` lru_cached (fallback warning logs once, not per tick), `ge=1` bounds on the three presence config fields, night-window edge test (gap wholly inside one night). PRD/ticket acceptance parenthetical extended to cover shift feed-through quantization (1e-4 in the nonzero-shift regime).

- 2026-07-17 - PR #104 review (P2, second finding): the threshold-crossing solver used the frozen stored shift, so a boundary state whose stored shift disagrees with f(load) (arousal 0.9, load 48 h, shift 0.05, noon, 8 h gap) overestimated t* by ~0.6 h and over-accumulated ~0.9 h vs composition. The solver now bisects A(t) composed via `relax_with_shift_dynamics` over the accumulation-regime shift segments (`_accumulation_shift_segments` — the +1 h/h load law holds exactly on any [0, t] with t ≤ t*, so the law is determined without knowing t*). Only remaining divergence vs composition is the documented tick quantization. Regression test includes RED evidence (the frozen-shift solve violates the accumulator quantum by ~0.9 h).
- 2026-07-17 - PR #104 review (P2): the active-user guard missed turns running longer than `presence_active_window_seconds` (stale `last_message_at` while the run is still generating). `_active_user_ids` now also counts users with a `RuntimeRun` in status "running", capped by new `presence_run_stale_seconds` (default 1800, ge=1) on `started_at` so a crashed run can't exclude a user from presence forever. Tests: fresh in-flight run skips despite stale last_message_at; stale running run does not block the tick.
