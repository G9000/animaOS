...
- Commands:
  ...
  - `cd apps/server && uv run pytest tests/test_inner_life_initiative.py -q --deselect tests/test_inner_life_initiative.py::test_fetch_ack_route_end_to_end` -> 87 passed, 1 deselected (the deselected e2e is the pre-existing env-drift failure below); `ruff check` on the two changed server files -> clean
