...
- 2026-07-08 00:55 MYT - Follow-up fix (found via full-suite run during ARH-012): backgrounding the feedback task in turn-context *assembly* (pre-invoke) raced the turn's own message persistence. The feedback task opens its own runtime session and commits; on a shared DB connection (StaticPool in tests; also a correctness risk under connection reuse) that commit interleaved with the in-flight `runtime_messages` INSERTs, colliding on `(thread_id, sequence_id)` and rolling back the turn — 3 `test_chat.py` cases failed (`persists_runtime_rows`, `tracks_turns`, `compacts_thread_context`), passing on p4 with these files stashed. Moved the feedback spawn to the Stage-4 post-turn hooks (`_run_post_turn_hooks`), which run only after `runtime_db.commit()`, guarded on a non-empty user message (the approval-resume path passes `""`). Still fully off the TTFT critical path. Full suite now 1973 passed with only the 8 documented pre-existing failures.
